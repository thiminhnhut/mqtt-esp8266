...
#define __LED2_PIN__ 12
#define __BUTTON2_PIN__ 13

WiFiSupport MQTT_WiFiSupport;
WiFiClient espClient;
PubSubClient client(espClient);
ControlLoad MQTT_ControlLoad(__LED2_PIN__, __ACTIVE_LED__);
Button MQTT_Button(__BUTTON2_PIN__, __ACTIVE_BUTTON__);
...
void loop() {
    ...
    if (MQTT_Button.get()) {
        stateLed = !stateLed;
        client.publish(__ESP2_PUB__, String(stateLed).c_str());
        delay(100);
    }
}
...
void reconnect() {
    while (!client.connected()) {
        ...
        if (client.connect(__ID_ESP2__)) {
            Serial.println("connected");
            client.subscribe(__ESP2_SUB__);
        } else {
